...
export default function Home(){
    ...
    async function handleSearch(page = 0){
        ...
        try {
            await axios.get(`https://www.googleapis.com/books/v1/volumes?q=${name == '' ? 'Bolsonaro' : name}&maxResults=${maxResults}&startIndex=${page}`)
            .then(response => {
                                    ...
                                    <div className="button-container">
                                        <button className="button" onClick={() => handleFavorites(selectedBook)}>
                                            {!!localStorage.getItem('@google-book-search/books') && localStorage.getItem('@google-book-search/books').indexOf(selectedBook.id) !== -1?
                                                <AiFillStar size={20} color="#FFF" style={{marginRight:5}} /> 
                                            :
                                                <FiStar size={20} color="#FFF" style={{marginRight:5}} /> 
                                            }
                                            Favoritos
                                        </button>
                                        <button className="button" onClick={() => handleMoreInfos(selectedBook)}>
                                            <FiPlusCircle size={20} color="#FFF" style={{marginRight:5}} /> Detalhes
                                        </button>
                                    </div>
                                </div>
                            ))}
                    ...
                    <div className="each-book book-details" >
                        <div class-name="photo-titles">
                            ...
                            <div className="title-subtitle">
                                <strong>Título: {book.volumeInfo.title}</strong>
                                <span>Autor: {book.volumeInfo.authors}</span>
                            </div>
                        </div>
                        {!!book.volumeInfo.description && <p><strong>Descrição:</strong> {book.volumeInfo.description}</p>}
                        {!!book.volumeInfo.language && <p><strong>Linguagem:</strong> {book.volumeInfo.language}</p>}
                        {!!book.volumeInfo.pageCount && <p><strong>Páginas</strong> {book.volumeInfo.pageCount}</p>}
                        {!!book.volumeInfo.publisher && <p><strong>Empresa:</strong> {book.volumeInfo.publisher}</p>}
                        {!!book.saleInfo.listPrice && <p><strong>Preço:</strong> R$ {book.saleInfo.listPrice.amount}</p>} 
                        ...
                        {!!book.volumeInfo.infoLink &&<p><strong>Informações:</strong> <a target="_blank" rel="noreferrer" href={book.volumeInfo.infoLink}>See it on google market</a></p>}
                        <button className="button" onClick={() => setInformations(false)}>
                            <FiXCircle size={20} color="#FFF"style={{marginRight:5, marginLeft: 10}} />Voltar
                        </button>
                    </div>
                )}
            </main>
